    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">{{ trans('Payment.delete_voucher') }}</h5>
            ...
            <form action="{{ route('Payment.destroy', 'test') }}" method="post">
                ...
                <div class="modal-body">
                    ...
                    <h5>{{ trans('Payment.warning_message') }} <span style="color: red"> {{ $payment->patients->name }}</span></h5>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ trans('Payment.close') }}</button>
                    <button type="submit" class="btn btn-success">{{ trans('Payment.submit') }}</button>
                </div>
            </form>
        </div>
    </div>
